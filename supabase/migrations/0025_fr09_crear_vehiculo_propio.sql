-- 0025_fr09_crear_vehiculo_propio.sql
-- Conductor registers their own vehicle (FR-09 self-service).

create or replace function public.crear_vehiculo_propio(
  p_patente text,
  p_marca text,
  p_modelo text,
  p_color text,
...
begin
  ...
  begin
    ...
    values (v_uid, v_patente, v_marca, v_modelo, v_color, p_capacidad)
...
revoke all on function public.crear_vehiculo_propio(text, text, text, text, int) from public;
grant execute on function public.crear_vehiculo_propio(text, text, text, text, int) to authenticated;
