...
import { registerOperadorSchema } from "@/domain/auth";
...
async function registerAction(
  ...
  const result = await signUpOperadorAction(formData);
  return result ?? null;
}
...
/** Alta self-service de operador (espejo conductor, sin DNI). */
export function RegisterOperadorWizard() {
  ...
  function validateCurrentStep(): boolean {
    let parsed;
    if (step === 1) {
      parsed = registerOperadorSchema.pick({ nombre: true }).safeParse({ nombre });
    } else if (step === 2) {
      parsed = registerOperadorSchema
        .pick({ apellido: true })
        .safeParse({ apellido });
    } else if (step === 3) {
      parsed = registerOperadorSchema
